...
async function processFilesAndCallAI(videoPath, audioPath) {
    console.log('进入 processFilesAndCallAI 函数');
    try {
        console.log('开始 AI 处理流程...');
        const env = checkEnvironmentVariables();
        
        // 检查视频处理模块
        console.log('检查视频处理模块导入状态:', 
            typeof processVideoFrames === 'function' ? '成功' : '失败',
            typeof readFileAsBase64 === 'function' ? '成功' : '失败'
        );
        ...
        console.log('视频文件:', videoFile.name, videoFile.size, '字节');
        console.log('音频文件:', audioFile.name, audioFile.size, '字节');

        // WebSocket URL
        const wsUrl = `${env.domain}${env.proxyPath}/v4/realtime?Authorization=${env.apiKey}`;
        console.log('准备连接 WebSocket:', wsUrl.replace(/Authorization=.*$/, 'Authorization=***'));
            ...
            try {
                ...
                ws.on('open', async () => {
                    try {
                        ...
                        // 4. 处理音频
                        console.log('开始处理音频...');
                        const audioBase64 = await readFileAsBase64(audioFile, (progress) => {
                            console.log(`音频处理进度: ${progress}%`);
                        });
